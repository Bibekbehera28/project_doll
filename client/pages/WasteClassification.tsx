...
const categoryMeta: Record<ManualCategory, { label: string; icon: React.ComponentType<any>; color: string; tips: string[] }> = {
  biodegradable: {
    label: 'Biodegradable',
    icon: Leaf,
...
const WasteClassification: React.FC = () => {
  ...
  const [manualCategory, setManualCategory] = useState<ManualCategory | null>(null);
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Upload a photo or select a category to get disposal guidance.</p>
        </div>
        <Badge variant="secondary">{modelReady ? 'ML Ready' : 'Mock Mode'}</Badge>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            ...
            <CardDescription>Drag and drop an image or use your camera</CardDescription>
          </CardHeader>
          <CardContent>
            <div
              {...getRootProps()}
              className={`border-2 border-dashed rounded-xl p-6 flex flex-col items-center justify-center text-center transition-colors ${
                isDragActive ? 'border-eco-primary bg-eco-primary/5' : 'border-border'
              }`}
            >
              <input {...getInputProps()} />
              ...
              <div className="mt-4 flex gap-3">
                ...
                <Input ref={inputRef} type="file" accept="image/*" capture="environment" className="hidden" onChange={async (e) => {
                  if (processing) return;
                ...
                >
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-2">
                      {result?.type && (
                        <>
                          {React.createElement(categoryMeta[result.type as ManualCategory].icon, { className: `w-5 h-5 ${categoryMeta[result.type as ManualCategory].color}` })}
                    ...
                    <div className="space-y-2">
                      ...
                      {result?.processingTime && (
                        <p className="text-xs text-muted-foreground">Processed in {result.processingTime} ms</p>
                      )}
                    </div>
                  </div>
                </motion.div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {(['biodegradable','recyclable','hazardous'] as ManualCategory[]).map((cat) => {
                const Icon = categoryMeta[cat].icon;
                const active = guideCategory === cat;
                return (
                  <Button key={cat} variant={active ? 'default' : 'outline'} className={active ? 'bg-gradient-to-r from-eco-primary to-eco-secondary text-white' : ''} onClick={() => setManualCategory(cat)}>
            ...
            <div className="mt-6">
              ...
              <div className="space-y-2">
                {guideCategory ? (
                  ...
                  <p className="text-sm text-muted-foreground">Upload an image or choose a category to see disposal tips.</p>
                )}
              </div>
            </div>
          </CardContent>
